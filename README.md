<div align="center">

# ⚡ Red-Eric

`Developer` · `Game Modder` · `Reverse Engineer`

---

</div>

### 🛠️ Tech Stack

**Languages**  
![C++](https://img.shields.io/badge/C%2B%2B-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)

**Reversing & Dev Tools**  
![IDA Pro](https://img.shields.io/badge/IDA_Pro-121212?style=flat-square&logoColor=white)
![x64dbg](https://img.shields.io/badge/x64dbg-222222?style=flat-square)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)

**Databases & Services**  
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-003B57?style=flat-square&logo=sqlite&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=flat-square&logo=firebase&logoColor=black)

---

### 🔧 What I'm Up To

```javascript
const redEric = {
  status: "Patching game binaries & building web tools",
  focus: ["Reverse Engineering", "Game Modding", "Full-Stack Dev"]
};
```

---
### 📊 GitHub Stats

<table width="100%">
  <tr>
    <td>
      <img src="https://github-readme-streak-stats.herokuapp.com/?user=Red-Eric&theme=radical" width="100%"/>
    </td>
  </tr>
  <tr>
    <td>
      <img src="https://github-stats-extended.vercel.app/api?username=Red-Eric&show_icons=true&theme=radical" width="100%" height="195"/>
    </td>
  </tr>
  <tr>
    <td>
      <img src="https://github-stats-extended.vercel.app/api/top-langs/?username=Red-Eric&layout=compact&theme=radical" width="100%" height="195"/>
    </td>
  </tr>
</table>
